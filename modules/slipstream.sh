...
slipstream_install() {
  ...
  if [[ ! -s /etc/slipstream/cert.pem || ! -s /etc/slipstream/key.pem ]]; then
    ...
    [[ "${HEXTUNNEL_DRY_RUN:-0}" == 1 ]] || chmod 600 /etc/slipstream/key.pem
    [[ "${HEXTUNNEL_DRY_RUN:-0}" == 1 ]] || chmod 644 /etc/slipstream/cert.pem
  fi
  if [[ ! -s /etc/slipstream/reset-seed ]]; then
    [[ "${HEXTUNNEL_DRY_RUN:-0}" == 1 ]] || openssl rand 32 > /etc/slipstream/reset-seed
    [[ "${HEXTUNNEL_DRY_RUN:-0}" == 1 ]] || chmod 600 /etc/slipstream/reset-seed
  fi

  local external_ip
  external_ip="$(ip -4 route get 1.1.1.1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src") {print $(i+1); exit}}')"
  [[ -n "$external_ip" ]] || external_ip="$(hostname -I | awk '{print $1}')"
...
EOF
  ...
  safe_restart_service slipstream "test -x $install_dir/target/release/slipstream-server && openssl x509 -in /etc/slipstream/cert.pem -noout"
...
slipstream_doctor() {
  ...
  port_is_listening udp 53 && printf open || printf closed
  printf ' udp5300='
  port_is_listening udp 5300 && printf open || printf closed
  printf '\n'
}
