...
HEXTUNNEL_OPERATION_LOCK_FILE="${HEXTUNNEL_OPERATION_LOCK_FILE:-/run/lock/hextunnel-operation.lock}"
...
if [[ -s "$HEXTUNNEL_VERSION_FILE" ]]; then
  ...
  HEXTUNNEL_VERSION="${HEXTUNNEL_VERSION:-development}"
fi
case "$HEXTUNNEL_VERSION" in
  *-rc.*|*-beta.*|*-alpha.*) HEXTUNNEL_RELEASE_CHANNEL="candidate" ;;
  development) HEXTUNNEL_RELEASE_CHANNEL="development" ;;
  *) HEXTUNNEL_RELEASE_CHANNEL="stable" ;;
esac
export HEXTUNNEL_VERSION HEXTUNNEL_RELEASE_CHANNEL HEXTUNNEL_OPERATION_LOCK_FILE
